...
tab1, tab2, tab3, tab4 = st.tabs(["Predict Insurance Cost", "Project Details", "Insurance Benefits", "Suitable Insurance Plan"])
...
with tab1:
    ...
    if st.button("Predict Insurance Cost"):
        ...
        st.success(f"Predicted Insurance Cost: {predicted_cost}")
...
with tab4:
    ...
    if st.session_state['predicted_cost'] is not None:
        ...
        # Suggest suitable plans based on the predicted cost
        if predicted_cost < 5000:
            st.write("### Basic Plan\n"
                     "- **Coverage**: ₹5,00,000\n"
                     "- **Premium**: ₹4,000 per year\n"
                     "- **Features**: Covers hospitalization, medical bills up to ₹5,00,000.")
        elif 5000 <= predicted_cost < 15000:
            st.write("### Standard Plan\n"
                     "- **Coverage**: ₹10,00,000\n"
                     "- **Premium**: ₹10,000 per year\n"
                     "- **Features**: Includes hospitalization, surgery, and maternity benefits.")
        else:
            st.write("### Premium Plan\n"
                     "- **Coverage**: ₹20,00,000\n"
                     "- **Premium**: ₹15,000 per year\n"
                     "- **Features**: Full coverage for hospitalization, critical illness, maternity, and other major treatments.")
    else:
