...
with tab1:
    ...
    # Define fixed price adjustments
    alcohol_yes_cost = 500  # Fixed cost if the user consumes alcohol
    alcohol_no_cost = 200   # Fixed cost if the user does not consume alcohol
    diabetes_cost = 600     # Additional cost if the user has diabetes
    bp_low_cost = 300       # Additional cost if BP is low
    bp_normal_cost = 100    # Additional cost if BP is normal
    bp_high_cost = 700      # Additional cost if BP is high

    # Additional questions for users above 40 years
    bp_status = None
    has_diabetes = 'no'
    bp_cost = 0  # Default BP cost if user is under 40
    if age > 39:
        # Collect blood pressure (BP) information
        bp = st.number_input("Enter your blood pressure (BP):", min_value=50, max_value=200, value=120)
        
        # Determine BP status and assign cost based on BP
        if bp < 80:
            bp_status = "Low"
            bp_cost = bp_low_cost
        elif 80 <= bp <= 120:
            bp_status = "Normal"
            bp_cost = bp_normal_cost
        else:
            bp_status = "High"
            bp_cost = bp_high_cost
        st.write(f"Your BP is considered: {bp_status}")
        
        # Ask about diabetes
        has_diabetes = st.selectbox("Do you have diabetes?", ['yes', 'no'])
    ...
    if st.button("Predict Insurance Cost"):
        ...
        # Calculate total cost with additional adjustments
        additional_cost = alcohol_yes_cost if alcohol == 'yes' else alcohol_no_cost
        if has_diabetes == 'yes':
            additional_cost += diabetes_cost  # Add diabetes cost if applicable
        total_cost = predicted_cost[0] + additional_cost + bp_cost  # Final total with BP adjustment
